...
from PyQt5.QtGui import QPixmap, QColor, QPen, QBrush, QPainter, QIcon
...
class Window(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.package_share_directory = get_package_share_directory('joystick_control_ras')
        self.setupUi()
    ...
    def loop_icons(self):
        # if rosnode has publisher, assume that broadcast is running. Loop between the icons
        if self.node.pub_actuation != None:
            if self.active_icon_ticker == 0 or self.active_icon_ticker == 3:
                self.setWindowIcon(self.icon_array[1])
                self.active_icon_ticker = 1
            elif self.active_icon_ticker == 1:
                self.setWindowIcon(self.icon_array[2])
                self.active_icon_ticker = 2
            elif self.active_icon_ticker == 2:
                self.setWindowIcon(self.icon_array[0])
                self.active_icon_ticker = 0
        else: # Just show the three bar icon otherwise
            self.setWindowIcon(self.icon_array[2])
            self.active_icon_ticker = 2
    ...
    def setupUi(self):
        ...
        self.icon_array = [
            QIcon(os.path.join(self.package_share_directory, 'resource', 'images', 'ras_broadcast_logo_partially_1bar.png')),
            QIcon(os.path.join(self.package_share_directory, 'resource', 'images', 'ras_broadcast_logo_partially_2bar.png')),
            QIcon(os.path.join(self.package_share_directory, 'resource', 'images', 'ras_broadcast_logo_partially_3bar.png'))
        ]
...
def main(args=None):
    ...
    # Define a signal handler to handle KeyboardInterrupt (Ctrl-C)
    ...
    # This causes the app to run until the window is closed
    shutdownstate = app.exec()

    # This causes the ROS node to shutdown, avoiding errors when the program is closed
    rclpy.shutdown()

    # Exit the application
    sys.exit(shutdownstate)
